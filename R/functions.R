...
# @param fun either a function or a non-empty character string
# naming the function to be called
# @param args a list of values to modify the default arguments
# of the function
# @param dots the user-specific input (usually from ...)
# @param keep.null \code{TRUE} to keep \code{NULL} values
# after argument modifications
...
# @details This function is used in vectorization when the names
# of an object is to be supplied. \code{NULL} value will break the
# vectorization while setting \code{def = character(1L)} makes the
# names vectorizable.
...
# @details
# A \code{NULL} value, zero-length vector or list have length zero, which
# is called empty.
# @param x the object
...
# have names as given by user. For example, \code{list.select} requires
# user to specify the fields to select. These fields are unevaluated
# arguments, some of which are symbols and others are calls. For the
# symbols, it is natural to make the resulted lists to have the same name
